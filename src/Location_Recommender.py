...
from Location_Optimizer import GetNewLocationRecommendations
...
class initiate(object):
	...
	def get_data(self):
		...
		"""
		df = pd.read_excel('data/1.3 Portfolio data for last three years full.xlsx', sheetname = 'Branch', skiprows = 3, \
		...
		for i in range(len(self._cities)):
			...
			time.sleep(.2) 		# delay of .1 second to not exceed query limit with geocode

		pass		

	def read_shapefile(self):
		"""
		Read in shapefile via Basemap
		"""
	...
	def plot_formatted_shapefile_with_locations(self, list_of_new_points):
		...
		dev = self._state_shapefile.scatter(
			...
			alpha = 0.9, antialiased = True, label = 'Current Office locations', zorder = 3)
		...
		dev = self._state_shapefile.scatter(
			...
			20, marker = 'o', lw = .25, facecolor = '#41ff16', edgecolor = 'w',
			alpha = 0.9, antialiased = True, zorder = 3, label = 'Recommendations')
...
# Reference:
# http://sensitivecities.com/so-youd-like-to-make-a-map-using-python-EN.html#.WJ_pbdArKAw