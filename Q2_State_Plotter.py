...
from Q2_Branch_Recommender import BranchRecommender
...
class initiate(object):
	...
	def get_data(self):
		...
		for i in range(len(self._cities)):
			# Getting Lat & Long for each city in self._state and appending it to self._df_cities_lat_long
	...
	def plot_formatted_shapefile_with_locations(self):
		plt.clf()
		fig = plt.figure()
...
if __name__ == '__main__':
	...
	start.map_df()
	start.plot_formatted_shapefile_with_locations()

	br = BranchRecommender(start._df_cities_lat_long, start.state_polygon, start._state_shapefile)
	br.initiate()
