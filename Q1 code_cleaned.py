...
from fraud_case_study import fraud_prediction as FP
...
def clean_df(df):
    ...
    weights = 20 + 10000000.0 * (good_df['Bouncing']/ (1.0 * good_df['Loan Amt']))
...
if __name__ == '__main__':
    ...
    pipeline = Pipeline([
        ...
        ('model', AdaBoost(n_estimators = 100))  # classifier
    ])
    parameters = {}
    ...
    print metrics.classification_report(y_test, y_preds) #, sample_weight = np.array(weights[test_indices]))
    # print metrics.confusion_matrix(y_test, y_preds)
    # print metrics.confusion_matrix(y_test, y_preds, sample_weight = np.array(weights[test_indices]))

    dump_models(best_model)

    fp = FP()
    costbenefit = np.array([[0, -1], [-50, 0]])  # TP=0, FP=-1, FN=-50, TN=0
    fp.plot_profit_models(costbenefit, X_train, X_test, y_train, y_test)
